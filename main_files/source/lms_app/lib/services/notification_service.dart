...
class NotificationService {
  ...
  Future initFirebasePushNotification(BuildContext context, WidgetRef ref) async {
    ...
    if (initialMessage != null) {
      await HiveService().saveNotificationData(initialMessage).then((value) => _navigateToDetailsScreen(context, initialMessage));
    }

    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      debugPrint('onMessage: ${message.data}');
      await HiveService().saveNotificationData(message).then((value) => _openNotificationDialog(context, message));
    });

    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
      debugPrint('onMessageOppn: ${message.data}');
      await HiveService().saveNotificationData(message).then((value) => _navigateToDetailsScreen(context, message));
    });
  }
